...
let nuevoArray = numeros.map((value) => value - 1);
...
let nuevosAnimales = animales.map((value) => value.toUpperCase());
console.log("Apartado B: ", nuevosAnimales); // ["VERTÍN, BIGOTES"]
//
//
// C) Usa la función map para actualizar con la fecha de hoy (new Date().toLocaleString())
//    la propiedad ultimaVisita de todos los pacientes de una clínica dental
...
let pacientesVisitados = pacientes.map(
  (paciente) => (paciente.ultimaVisita = new Date().toLocaleDateString())
);
...
//    El campo debe llamarse 'avisos'. Si el paciente NO ha pagado la última visita,
//    dicho campo debe contener el string 'Paciente moroso, cobrar!'; en caso contrario, debe dejarse vacío.
...
let pacientesAvisos = pacientes.map((paciente) => {
  paciente.avisos =
    paciente.ultimaVisitaPagada === true ? "" : "Paciente moroso, cobrar!";
  return paciente;
});
